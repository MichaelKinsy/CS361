...
public class DFA implements DFAInterface{
	...
	public void addTransition(String fromState, char onSymb, String toState){
		...
		if(from == null){
			System.err.println("ERROR: No DFA state exists with name " + fromState + " FROMSTATE");
			System.exit(2);
		} else if (to == null){
			System.err.println("ERROR: No DFA state exists with name " + toState + " TOSTATE");
			System.exit(2);
		}
	...
	public Set<DFAState> getFinalStates() {
		Set<DFAState> ret = new  HashSet<DFAState>();
		for(DFAState s : states){
			if(s.isFinal()){
				ret.add(s);
			}
